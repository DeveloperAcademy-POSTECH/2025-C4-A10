...
def check_dir(save_directory, do_predict, reverse_check_num=10):
    if not os.path.isdir(f'save/{save_directory}'):
        ...
        os.path.isdir(f'save/{save_directory}') and 
        not bool(glob(f'save/{save_directory}/*.pt'))
    ): # 디렉토리는 생성했지만 저장한 모델이 없는 경우
        return save_directory
    
    elif (
        os.path.isdir(f'save/{save_directory}') and 
        bool(glob(f'save/{save_directory}/*.pt')) and 
        not bool(glob(f'save/{save_directory}/*.csv'))
