...
def wandb_setting(config: DictConfig, sweep: bool = False):
    ...
    assert config.get("wandb"), (
        "Please write wandb configuration in your config.yaml file"
    )
    ...
    try:
        ...
        if not key:
            raise ValueError("Please set WANDB_TOKEN in .envs file")
        ...
        if config.wandb.use_wandb:
            ...
            print(
                "=" * 50,
                "use_wandb = False이므로 wandb를 사용하지 않습니다.",
                "=" * 50,
                sep="\n",
            )
            return False
        ...
        print(
            "=" * 50,
            "에러가 발생하여 wandb 설정을 off하고 진행합니다. 에러를 체크해주세요.",
            "=" * 50,
            sep="\n",
        )
        print(e)
